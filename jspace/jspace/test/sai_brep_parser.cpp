...
namespace jspace {
  namespace test {
      ...
      throw(std::runtime_error)
    {
      ...
      if (doc.Error())
	throw runtime_error("jspace::test::BRParser::parse(" + fileName
	...
	robot_->defaultJointPosVec_.resize(robot_->numJoints_);
	robot_->upperJointLimitVec_.resize(robot_->numJoints_);
	robot_->lowerJointLimitVec_.resize(robot_->numJoints_);
	...
	// FOUND TAG "type":
	  ...
	  default:
	    throw std::runtime_error("jspace::test::BRParser::exploreJointNode(): invalid <type> `"
	...
	// FOUND TAG "axis":
	  ...
	  default:
	    throw std::runtime_error("jspace::test::BRParser::exploreJointNode(): invalid <axis> `"
